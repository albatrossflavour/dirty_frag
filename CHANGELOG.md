...
- Structured custom fact (`dirty_frag`) reporting `loaded`, `blacklisted`, and `available` state for the esp4, esp6, and rxrpc kernel modules
- Puppet class (`dirty_frag`) with opt-in Boolean parameters (`mitigate_esp4`, `mitigate_esp6`, `mitigate_rxrpc`) to blacklist vulnerable modules via `/etc/modprobe.d/dirtyfrag.conf`
